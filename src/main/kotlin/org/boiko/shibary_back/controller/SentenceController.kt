...
class SentenceController(private val sentenceService: SentenceGenerationService) {
    ...
    @PostMapping
    fun generate(@RequestBody request: SentenceRequest): ResponseEntity<Any> {
        ...
        if (request.count !in MIN_COUNT..MAX_COUNT) {
            return ResponseEntity.badRequest().body(mapOf("error" to "Count must be between $MIN_COUNT and $MAX_COUNT"))
        }
        val result = sentenceService.generate(request.word, request.count)
